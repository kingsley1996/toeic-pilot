...
| ~~**_corpus đối chiếu 2024_**~~ | **Xong test 1.** `tp-2024-01`: 200/200 câu (audio + ảnh P1 từ sách, nạp dev). Ba script tái hiện cho test 2–10 + checker cấp câu | `EXTRACT-REF-2024.md` |
...
| **`tp-form-14`** | **Đã sync lên production dưới dạng draft (2026-09-13): 200 câu · 31 audio câu + 23 audio cụm · 6 ảnh Part 1 + 8 graphic · giải thích 200/200 · đáp án cân 28/25/24/23.** Viết tay toàn bộ 200 câu (agent đóng vai model, không gọi provider ngoài): blueprint qua builders đúng luồng `cmd_plan`, `check` 0 chặn 0 cờ, verify tay 200/200 đáp án + 8/8 verdict hình thay cho `--verify` (bỏ qua theo yêu cầu). Ảnh P1 user vẽ ngoài từ `*.prompt.txt`, đã xem trước khi gắn. Sự cố: writer song song ghi đè file giữa chừng (p6-04 nhân đôi đoạn văn — checker mù với lỗi này, đã sửa tay); p1-02 sửa option sau nạp qua PATCH `audio_script` + thu lại clip ("Two workers" → "Workers" vì ảnh có ba người). Còn: `--verify` chính thức, publish qua admin UI | `EXAM-GENERATION-RUNBOOK.md`, `SYNC-TEST-TO-PRODUCTION.md`, `EXAM-WRITE-GUIDE.md` |
| **`tp-form-15`** | **Đã sync lên production dưới dạng draft (2026-09-13): 200 câu · 31 audio câu + 23 audio cụm · 6 ảnh Part 1 + 8 graphic · giải thích 200/200 · đáp án cân 27/26/24/23 · thể tích 6 897 từ (+13% họ đề), ngoài-10k 11.0%.** Viết tay theo `EXAM-WRITE-GUIDE` (seed 1605618571, không provider ngoài): `check` 0 chặn 0 cờ, `compare` 0 trùng khít (max-sim 0.48), quota từ hiếm đạt ngay lần đầu. Ảnh P1 **lần đầu tự sinh bằng mflux qua `_imagegen`** (~3 phút/tấm), user duyệt trước khi gắn. Verify tay hai vòng; vòng hard-review bắt 4 lỗi mà `check` mù: p7-12 hai cột biểu đồ cùng >4 (hai đáp án đúng), p7-14 đáp án tự mâu thuẫn số học (toa 8 chỗ ≥ nhóm 5), p7-13 nhiễu paraphrase lệch nguyên văn, p7-15 lựa chọn rác sót từ vá leak — tất cả đã sửa. Bài học ghi vào guide: `--verify` và đọc lại hard là hai việc khác nhau. Còn: publish qua admin UI | `EXAM-WRITE-GUIDE.md`, `EXAM-GENERATION-RUNBOOK.md` |
...
| ~~**Sửa ngữ liệu văn bản trên admin**~~ | **Xong (2026-09-14).** `PATCH /admin/question-sets/{id}` nhận thêm `passages: [{slot, text}]` (Part 6 chỉ ô 1, Part 3/4 từ chối — cùng kiểu chặn "lời thoại thứ hai"); ô trống là None, sửa hạ cả cụm lẫn câu về nháp, không chạm vân tay audio. Màn đề có nút Sửa từng ô (đóng chỉ khi server nhận, lỗi in cạnh nút — cùng pattern AudioPanel). 3 test `test_admin_api.py`, contract regen | `ADR-007` |
