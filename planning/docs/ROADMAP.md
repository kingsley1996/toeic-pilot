...
| ~~**Ngữ pháp — module thứ năm, G1–G5**~~ | **Xong.** Năm bảng (migration 057–060), admin CRUD + màn soạn bài theo trang riêng với ô xem trước, cây học `/learn/grammar`, bài `practice` gắn câu từ kho nhãn hoặc soạn tại chỗ, tiến độ bằng nút hoàn thành ghi bảng (không suy ra từ `attempt` — lý do đảo ở spec), chuỗi `next_topic` nối chủ đề nền tảng không mã với chủ đề theo nhãn. G5 (migration 061): một câu đúng = 2 XP tất định theo (người, câu), khe "Học ngữ
  pháp" 3 bài kẹp theo số bài còn lại, và `grammar_attempt` tính vào chuỗi ngày +
  quà ruby. G3 (luyện theo nhãn cuối chủ đề) từng dựng rồi **bỏ**. markdown-lite 81 → 242 dòng. **Còn**: 14/18 chủ đề `draft` không bài | `SPEC-GRAMMAR.md` |
