...
- **`planning/ADR-013-PET-CONDITION.md`** — ba chỉ số của thú cưng nghĩa là gì, và độ hiếm khác nhau ở đâu. Đọc §0 trước khi đụng vào góc thú cưng: `PetView` **không có trường nhu cầu nào**, nên hôm nay con thú đói và con thú no được vẽ giống hệt nhau, và cột "Vui" không ảnh hưởng tới bất cứ thứ gì. Hai ràng buộc chốt ở §1 và cả hai đều dễ bị "cải thiện" ngược lại: **chỉ số không chạm ra ngoài bảng** (không ruby, không XP, không nhịp chạm mặt) và **độ hiếm khác ở hành vi, không ở sức mạnh**.
...
- **`planning/ADR-014-DEPLOY-FREE.md`** — how this goes to production on free tiers with **no credit card**: Vercel + Render + Supabase + Upstash + Cloudinary, and why every other candidate was rejected. Its §0 is the part worth reading first — the deployment needs *no* architectural compromise, and that is a dividend of four decisions already made for other reasons (storage provider is config, the API never serves media bytes, the worker is a separate image, Redis is soft). Three things there fail silently: Supabase's **session pooler on 5432**, not the transaction pooler (psycopg3's prepared statements break on 6543, and only from the sixth call of a query); one ping at **`/ready`** keeps Render, Supabase and Upstash awake at once, because it already touches all three; and 750 free hours against a 744-hour month leaves room for exactly **one** always-on service. The limit that runs out first is **Redis**, not bandwidth — `get_current_user` spends one `EXISTS` per authenticated request. §7 records why the two halves ship differently and is the part most likely to be "tidied" backwards: the API runs a **pre-built image from GHCR** so what production runs is the image CI actually booted (and an image-backed Render service does **not** redeploy on a new tag — CI must call the deploy hook), while the web cannot be Dockerised at all — Vercel runs no containers, and moving it to Render needs 1 488 of the 750 free hours. Every number in it was measured on 2026-08-28 and will rot. **`/admin/system` draws this topology live** (`GET /admin/system/status`, `require_role("admin")`): Postgres and Redis are reported by the API because the API calls them, while audio and images are probed **from the browser**, because that is the path they actually take — ADR-006 §2.9 keeps media off the API entirely, so a server-side check there would test a route nobody walks. One rule on that endpoint fails silently and is pinned by `tests/test_admin_system.py`: the provider host shown per dependency comes from a connection string, so it must have `user:password@` stripped — the first version printed the production database password onto a page built to be shown to people.
...
- **`planning/PROMPT-SYSTEM.md`** — kiểm kê mọi prompt gửi cho mô hình, phân theo mục đích. Đọc §0 trước: có **hai họ prompt** và ranh giới giữa chúng là ranh giới kiến trúc. **Hai sổ đăng ký `.md`, cùng lớp `Prompt`, hai thư mục — tách cố ý**: `app/services/llm/prompts/` (4, chạy trong request) và `app/content/exam/prompts/` (11, chạy ngoài luồng, vào qua `_registry.exam_prompt`). Cả hai đánh phiên bản bằng **hash của chính nội dung**, nên không có đường nào sửa prompt mà quên tăng số; chỉ sổ runtime mới ghi `ai_interaction.prompt_version`, và gộp hai sổ làm ranh giới ấy vô hình. Chỗ dễ hỏng im lặng khi sửa: prompt `.md` giữ **mốc hợp đồng** dạng `{BLANK}`, `{PHOTO_MARKER}` — chép cứng giá trị vào tệp thì đổi `contract.py` sẽ làm prompt lệch khỏi trình phân tích mà không ai biết; `render` dùng `str.format` nên thiếu khoá là nổ chứ không im. `graphic.py` và `check_prompts.py` **cố ý ở lại Python**: cái đầu là template hai tầng, cái sau prompt ngắn và đã là mô-đun riêng.

Three descriptions of *current behaviour* rather than of decisions:
...
- **`planning/EXAM-GRAPH.md`** — how the exam-generation graph actually runs, part by part, plus a copy-paste runbook in §10. Two entry points and they differ in a way that costs data: `exam_agents.full` is `plan → slotloop → balance → spread`, while `exam_agents.graph` is the per-slot loop **only** — no `balance` — so running part by part through `graph` leaves the answer letters unbalanced, and `balance` must run **before** `load` because it rewrites the paste files. §3 is the part worth reading before touching any per-part code: Part 2 has **three** options not four, Part 3 asks its graphic question at the **third** question while Part 4 asks at the **second** (hardcoding `questions[-1]` checks the wrong question in Part 4 and still returns a verdict), and Part 7's slots hold **2 to 5** questions each with their graphics on the *passage* rather than the slot. The queue is a directory query, so re-running any command writes only what is missing — but an `escalated` slot keeps its paste file and is therefore never picked up again; delete the file to retry it. With a mandatory-reasoning model `--max-tokens 40000` is required: the reported "N ký tự suy nghĩ" in a truncation error measures the cap that was hit, not what the model needed.
...
uv run python -m app.content.sync_kb       # knowledge base: file md -> bảng knowledge_chunk
uv run python -m app.content.embed_kb --create-index  # embed + đẩy vector lên Pinecone
...
Tests default to SQLite in-memory (`tests/conftest.py` sets `DATABASE_URL` before the app imports). Tests marked `integration` need real PostgreSQL and **skip automatically** when it is unreachable. **Point them at a scratch database — they fail closed:** `test_concurrency.py` and `test_ruby_race.py` **refuse to run without `TEST_DATABASE_URL`** (`pytest.skip` at module level), so a bare `pytest` never writes to the dev database again. It did once: `test_ruby_race.py` DELETEs the pet tables full-table to set up a race, and without the guard a bare `pytest` wiped the dev collection (2026-08-30). CI runs them for real against a Postgres service.
...
**Keep comments short.** One or two sentences is the normal size. Comment only where the code cannot speak for itself: an invariant that fails silently, a non-obvious ordering, a decision whose obvious alternative is wrong. Do not restate what the code already says, and do not narrate the history of the attempts that led to the current line.

This repository does value *why* over *what* — several bugs here fail silently, and the reason a line exists is worth recording. That is an argument for putting a reason in a few places, not for putting a paragraph everywhere. When the full reasoning is worth keeping, it belongs in `planning/` — an ADR or `ROADMAP.md` — and the comment points at it in one line rather than repeating it.

A comment growing past a few lines is usually a signal about something else: either the code should be clearer, or the explanation belongs in a planning document.
