...
**The frontend has a design system, not per-page styling.** `src/components/ui.tsx` holds the primitives, colours come from CSS variables in `globals.css` (so light and dark are one definition rather than a `dark:` twin on every element), and `src/lib/session.tsx` resolves who is signed in *once* for the whole app. Four things follow from that and are easy to undo by accident:

- **A failed `/auth/me` is not a rejected token, and only the server may end a session.** `SessionProvider` used to call `clearAccessToken()` in the `.catch()` of that one fetch, so *any* failure signed the user out — a network blip, a 503, or, by far the most common, the request being aborted by the navigation that was already underway. Three reloads in quick succession were enough: the user landed on `/login` after a fast click, with nothing logged anywhere. It now clears only on `ApiError` 401/403. Keeping the token is only half the fix, because `status` is derived from `user`: a read that fails and is never retried leaves the whole app at `loading` forever, which is a different silent failure rather than none, so a transient error retries twice with a short backoff and then gives up. `e2e/auth.spec.ts` pins **both** directions by intercepting `/auth/me` — one aborted call must not sign anyone out, and a real 401 must.
