...
def cmd_attach_images(args: argparse.Namespace) -> int:
    ...
    with SessionLocal() as session:
        ...
        for part, directory, match in jobs:
            ...
            # Attribution mặc định phân theo KIND, không theo part số: Part 1 là
            # ảnh mô hình sinh; Part 3/4/7 là bảng biểu đồ **vẽ bằng code từ dữ
            # liệu** (ADR-004), dán nhãn "AI-generated" lên chúng là nói sai. Cả
            # hai chuỗi đều chứa token "generated" viết thường nên credit() (web)
            # gộp license về một dòng, không thành "… · generated".
            default_attribution = (
                "TOEIC Pilot — generated from data" if is_partial else "AI-generated by TOEIC Pilot"
            )
            try:
                done = import_images(
                    session,
                    ...
                    attribution=args.attribution or default_attribution,
