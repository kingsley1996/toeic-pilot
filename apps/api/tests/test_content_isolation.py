...
`content` extra, so it has no edge-tts and no mutagen. If anything reachable from
`app.main` ever imports `app.content`, the image stops booting — and it does so at
container start, not at build time, which is a slow and expensive way to find out.
...
def test_app_main_does_not_import_the_content_pipeline() -> None:
    ...
    assert not leaked, (
        f"app.main pulls in {leaked}. The production image has no edge-tts or "
        f"mutagen, so this breaks container startup. Move the shared code into "
        f"app/core/ instead."
    )
